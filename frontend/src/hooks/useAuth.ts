...
export function useAuth() {
  const { state, login, register, logout, refreshUserProfile, clearError } = useAuthContext();
  ...
  return {
    ...
    userDisplayName: state.user?.display_name || state.user?.username || state.user?.email || '',
    ...
    isSubscribed: (): boolean => {
      return !!(state.user?.subscription_type && state.user.subscription_type !== 'free');
    },
  };
} 