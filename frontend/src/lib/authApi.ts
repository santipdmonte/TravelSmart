...
async function authApiRequest<T>(
  ...
  try {
    ...
    if (!response.ok) {
      return {
        error: data.message || data.detail || `HTTP error! status: ${response.status}`
      };
    }
 ...
 */
export async function refreshToken(refreshTokenValue?: string): Promise<ApiResponse<TokenData>> {
  ...
  return authApiRequest<TokenData>(`/auth/refresh-token?refresh-token=${encodeURIComponent(tokenToUse)}`, {
    method: 'GET',
  });
}
 ...
 */
export async function verifyEmail(token: string): Promise<ApiResponse<EmailVerificationResponse>> {
  return authApiRequest<EmailVerificationResponse>(`/auth/verify-email?token=${encodeURIComponent(token)}`, {
    method: 'GET',
  });
}
 ...
 */
export async function resendVerification(): Promise<ApiResponse<{ message: string }>> {
  return authenticatedRequest<{ message: string }>('/auth/resend-verification', {
    method: 'POST',
  });
}
